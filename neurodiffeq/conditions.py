...
class DirichletIBVP1D_test(BaseCondition):
    r"""EC ADDITION: FOR THE WAVE EQ, OUT OF DATE COMMENTS
    An Dirichlet boundary condition on the boundary of :math:`[x_0, x_1] \times [y_0, y_1]`, where
    ...
    def __init__(self, x_min, x_min_val, x_max, x_max_val, t_min, t_min_val, t_min_prime, t_max):
        r"""Initializer method
        """
        super().__init__()
        self.x0, self.f0 = x_min, x_min_val
        self.x1, self.f1 = x_max, x_max_val
        self.t0, self.g0, self.gprime = t_min, t_min_val, t_min_prime
        self.t1 = t_max

    def parameterize(self, output_tensor, x, t):
        r"""EC ADDITION: FOR THE WAVE EQ, OUT OF DATE COMMENTS
        ...
        """
        x_tilde = (x - self.x0) / (self.x1 - self.x0)
        t_tilde = (t - self.t0) / (self.t1 - self.t0)
        Axt = torch.sin(np.pi * x)
        u_adj = Axt + x_tilde * (1 - x_tilde) * (1 - torch.exp(-t_tilde ** 2)) * output_tensor
