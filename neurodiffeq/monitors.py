...
class BaseMonitor(ABC):
    ...
    @staticmethod
    def _refresh_history(ax, history, lines_dict,  x_label='Epochs', y_label=None, title=None):
        ax.set_title(title)
        ...
        for metric, values in history.items():
            ...
            if metric not in lines_dict:
                line, = ax.plot(x, values, label=metric)
                lines_dict[metric] = line
                ax.legend()
            else:
                lines_dict[metric].set_data(x, values)
    ...
    def _split_history(self, history):
        ...
        for k, v in history.items():
            if k in ["train_loss", "valid_loss"]:
                loss[k] = v
            elif "__" in k and "loss" in k:
                components[k] = v
            else:
                metrics[k] = v
...
class MonitorSpherical(BaseMonitor):
    ...
    def check(self, nets, conditions, history, analytic_mse_history=None):
        ...
        loss_history, component_history, metric_history = self._split_history(history)
        
        n_aux_rows = sum(bool(d) for d in [
            loss_history,
            component_history,
            metric_history
        ])
        ...
        n_row = n_vars + n_aux_rows
        n_col = 3

        if self.fig is None:
            self.fig = plt.figure(figsize=(24, 6 * n_row))
            self.axs = np.atleast_2d(self.fig.subplots(nrows=n_row, ncols=n_col, gridspec_kw={'width_ratios': [1, 1, 2]}) )
            ...
            if loss_history:
                current_row += 1
                self.ax_loss = self.fig.add_subplot(n_row, 1, current_row)
            if component_history:
                current_row += 1
                self.ax_components = self.fig.add_subplot(n_row, 1, current_row)
            if metric_history:
                current_row += 1
                self.ax_metrics = self.fig.add_subplot(n_row, 1, current_row)
...
class Monitor1D(BaseMonitor):
    ...
    def check(self, nets, conditions, history):
        ...
        idx = 0
        self.ax_solution = self.axs[idx]
        idx += 1
        
        if loss_history:
            self.ax_loss = self.axs[idx]
            self.ax_loss.set_yscale('log')
            idx += 1
        
        if component_history:
            self.ax_components = self.axs[idx]
            self.ax_components.set_yscale('log')
            idx += 1
        
        if metric_history:
            self.ax_metrics = self.axs[idx]
            self.ax_metrics.set_yscale('log')
        
        
        self.ax_solution.clear()
        for i, u in enumerate(us):
            self.ax_solution.plot(self.ts_plt, u, label=f'variable {i}')
        self.ax_solution.legend()
        self.ax_solution.set_title('solutions')
...
# class BaseMonitor(ABC):
    ...
    # def _refresh_history(ax, history, x_label='Epochs', y_label=None, title=None):
        ...
        # #xlength =  max([len(v) for k,v in history.items()])
...
# class Monitor1D(BaseMonitor):
    ...
    # def check(self, nets, conditions, history):
        ...
        # if self.fig is None or len(self.axs) != n_cols:
            # plt.close(self.fig) if self.fig is not None else None
