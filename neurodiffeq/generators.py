...
class SamplerGenerator(BaseGenerator):
    ...
    def get_examples(self) -> List[torch.Tensor]:
        ...
        self.examples_history.append([sample.detach().cpu().numpy() for sample in samples])
        return samples
    ...
    def get_entropy(self):
        return _get_entropy_from_generator(self.generator)
...
def _get_entropy_from_generator(generator):
    size = generator.size
    ...
    generator.prob_fn = prob_fn.detach().cpu().numpy()
    generator.centers = gaussian_h.centers.detach().cpu().numpy()
...
class NNGenerator1D(BaseGenerator):
    def __init__(self, size, t_min=0.0, t_max=1.0, C1=0.1, method='equally-spaced',
        ...
        self.z_size = z_size
        self.loss = loss
        self.net = FCNN(n_input_units=z_size, n_output_units=size, hidden_units=hidden_units, actv=nn.LeakyReLU,
                        last_actv=nn.Tanh)
        self.metrics_history = {'generator_loss': []}
        self._requires_training = True
        self.generator_loss = 0.0

    def _sample_from_generator(self):
    ...
    def calculate_loss(self, examples):
        ...
        if self.loss == 'variance':
            ...
            for i in range(num_examples // self.size):
                more_examples = torch.cat((more_examples, self._sample_from_generator()))
            self.me = more_examples
            gaussian_h = GaussianHistogram(bins=self.size, min=self.t_min, max=self.t_max,
                                           sigma=(self.t_max - self.t_min) / (2 * self.size))
            hist_tensor = gaussian_h(more_examples)
    ...
    def get_examples(self):
        examples = self._sample_from_generator()
...
class NNGenerator2D(BaseGenerator):
    def __init__(self, size, xy_min=(0.0, 0.0), xy_max=(1.0, 1.0), C1=0.1, method='equally-spaced',
        ...
        self.z_size = z_size
        self.loss = loss
        self.net = FCNN(n_input_units=z_size, n_output_units=2 * size, hidden_units=hidden_units, actv=nn.LeakyReLU,
                        last_actv=nn.Tanh)
        self.metrics_history = {'generator_loss': []}
        self._requires_training = True
        self.generator_loss = 0.0
